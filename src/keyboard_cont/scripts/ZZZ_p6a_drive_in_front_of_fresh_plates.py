...
if __name__ == "__main__":
    rospy.init_node('ZZZ_p6a_drive_in_front_of_fresh_plates') #make node 
    rate=rospy.Rate(10)
    ...
    while not rospy.is_shutdown() and current_pose.y<1.35:
        twist.linear.x = -0.1
        twist.angular.z = 0.0
        pub.publish(twist)
        rate.sleep()
    ...
    rospy.sleep(1)
    '''
    while not rospy.is_shutdown() and current_pose.x<0.3:
        twist.linear.x = 0.2
        twist.angular.z = 0.0
        pub.publish(twist)
        rate.sleep()
    ...
    rospy.sleep(1)
    '''
    os.system("rosnode kill ZZZ_p6a_drive_in_front_of_fresh_plates")
